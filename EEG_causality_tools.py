...
#remember: scikit-learn (sklearn) and EDFlib-Python also needed in mne functions
            ...
            if raw1 is not None:
                ...
                def update_chan():
                    ...
                    def do_reordering():#this function is the channel reordering, which is called after sorting out the stimulus channel  
                        ...
                        window2.destroy()
                    stim1=None
                    if stim_chan.get()!='Not connected': #this part within "if" is the automatic event detection from photodetector.
                        window2.destroy()
                        t1=raw1.times
                        ...
                        Label(winstim,text="Minimum duration of events").grid(row=10,column=0,padx=10,sticky=W,columnspan=2)
                        ...
                        if raw2 is not None:
                            ...
                            Label(winstim,text="Minimum duration of events").grid(row=20,column=0,padx=10,columnspan=2,sticky=W)
...
def preprocess():
    ...
    if raw1 is None:
        ...
        def step1():
            ...
            def step2():
                ...
                def step3():
                    ...
                    def step4():
                        ...
                        def step5():
                            ...
                            Entry(win8,textvariable=chan_ref_exc,width=20).grid(row=2,column=0,padx=10,sticky=W)
                            Radiobutton(win8,text="Average of the following channels:",variable=ref_opt,value=3).grid(row=3,column=0,padx=10,sticky=W)
                            Entry(win8,textvariable=chan_ref_inc,width=20).grid(row=3,column=0,padx=10,sticky=W)
                            Radiobutton(win8,text="Using Reference Electrode Standardization Technique (REST) to a point at infinity",textvariable=ref_opt,value=4).grid(row=4,column=0,padx=10,sticky=W,columnspan=2)
                            ...
                            def do_reref():
                                ...
                                if ref_opt.get()==1:
                                    ...
                                    raw1=raw1.copy().set_eeg_reference('REST')
                                    if raw2 is not None:
                                        raw2=raw2.copy().set_eeg_reference('REST')
                                win8.destroy()
                                ...
                                if events1_data is not None:
                                    ...
                                    data1=np.append(raw1.get_data(),events1_data)
                                    ...
                                    if raw2 is not None:
                                        ...
                                        data2=np.append(raw2.get_data(),events2_data)
                                ...
                                def step6():
                                    ...
                                    def epoching():
                                        if whole_part_event==1:
                                            epoched_raw1=mne.Epochs(raw1,events1,tmin=float(tstart_whole.get()),tmax=float(duration_whole.get()),event_id=event_dict,preload=True,baseline=None)
                                            if raw2 is not None:
                                                epoched_raw2=mne.Epochs(raw2,events2,tmin=float(tstart_whole.get()),tmax=float(duration_whole.get()),event_id=event_dict,preload=True,baseline=None)
                                        elif whole_part_event==2:
                                            ...
                                            for (tmin,tmax) in split_times:
                                                ...
                                                split_raw1[-1].drop_bad(reject=reject_criteria,flat=flat_criteria)
                                                if len(split_raw)>1:
                                            ...
                                            if raw2 is not None:
                                                ...
                                                for (tmin,tmax) in split_times:
                                                    ...
                                                    split_raw2[-1].drop_bad(reject=reject_criteria,flat=flat_criteria)
                                                    if len(split_raw)>1:
                                ...
                                step6()
                            Button(win8,text="OK",command=do_reref).grid(row=5,column=0,padx=10,columnspan=2)
                            raw1.plot_sensors()
                        if do_ica.get()==1:
                            ...
                            Label(win5,text="Inform ICA components to delete (separate with commas):").grid(row=1,column=0,padx=10)
                            ...
                            def cont_ica():
                                ...
                                Label(win6,text="Inform ICA components to delete (separate with commas):").grid(row=1,column=0,padx=10)
                                ...
                                def end_ica():
                                    ...
                                    if to_remove2[0]!='':
                                      ...
                                      if raw2 is not None:
                                        ...
                                        Label(win7,text="Inform ICA components to delete (separate with commas):").grid(row=1,column=0,padx=10)
                                        ...
                                        def cont_ica2():
                                            ...
                                            Label(win8,text="Inform ICA components to delete (separate with commas):").grid(row=1,column=0,padx=10)
                                            ...
                                            ica2_2.plot_sources(temp2)
                                            ica2_2.plot_properties(temp2,picks=list(range(len(temp2.ch_names))))
                                        Button(win7,text="OK",command=cont_ica2).grid(row=3,column=0,padx=10)
                                        ica_2.plot_sources(raw2)
                                        ica_2.plot_properties(raw2,picks=list(range(len(raw2.ch_names))))
                                      else:
                                        showinfo(title="info",message="ICA filtering completed\nProceeding with re-referencing")
                                        step5()  
                                Button(win6,text="OK",command=end_ica).grid(row=3,column=0,padx=10)
                                ica2.plot_sources(temp)
                                ica2.plot_properties(temp,picks=list(range(len(temp.ch_names))))
                            Button(win5,text="OK",command=cont_ica).grid(row=3,column=0,padx=10)
                            ica.plot_sources(raw1)
                            ica.plot_properties(raw1,picks=list(range(len(raw1.ch_names))))
                        else:
                            step5()
                ...
                if hasdbs.get()==1 or hasdbs.get()==3:
                    ...
                    Label(win3,text="Sensitivity (0 to 1):").grid(row=2,column=0,padx=10,sticky=W)
                    ...
                    def peakfind1():
                        ...
                        pk,prop=find_peaks(sig,distance=100,prominence=np.quantile(sig,float(quantile1.get())))
                ...
                if hasdbs.get()==2 or hasdbs.get()==3:
                    ...
                    Label(win3,text="Sensitivity:").grid(row=2,column=0,padx=10,sticky=W)
                    ...
                    def peakfind2():
                        ...
                        pk,prop=find_peaks(sig,distance=100,prominence=np.quantile(sig,float(quantile2.get())))
                        wd,wdht,left_ips,right_ips=peak_widths(sig,pk)
                        sig2=10*np.log10(sig)
                        wdht2=10*np.log10(wdht)
                        plt.plot(f2,sig2)
        ...
        raw1.plot_sensors(show_names=True)
        raw1.plot_psd_topo(fmin=1,fmax=50,color='k',fig_facecolor='w',axis_facecolor='w')
        if raw2 is not None:
            raw2.plot_psd_topo(fmin=1,fmax=50,color='k',fig_facecolor='w',axis_facecolor='w')
        win.lift()
...
def make_frame2(y,vmin=-1,vmax=1,color='bwr',lblttl='Pearson correlation',frttl='my_frame.png'):
    ''' y     = dataframe for eeg data
        ...
        frttl = frame title'''
    fig, axes = plt.subplots(nrows=1, ncols=1)
    sns.heatmap(y1,xticklabels=y1.index,yticklabels=y1.columns,mask=y1.isnull(),cmap=color,ax=axes,vmin=vmin,vmax=vmax)
    plt.xlabel("Electrodes")
    plt.ylabel("Electrodes")
    axes.figure.axes[-1].set_ylabel(lblttl)
    fig.savefig(frttl,dpi=300)
            ...
            if (float(fmin.get())>0) and (float(fmax.get())>float(fmin.get())) and (int(nfreq.get())>0):
                ...
                for key in event_dict.keys():
                    ...
                    if x2 is not None:
                        ...
                        pbar['value'] += 100/pmax
                        pbtxt['text']=f"{k:d}/{pmax:d}"
                        a2,b2 = mne.time_frequency.tfr_morlet(x1[key], freqs=freqs, n_cycles=n_cycles, use_fft=True,return_itc=True, decim=3, n_jobs=1)
                        power2.append(a2)
                        itc2.append(b2)
                def make_plot():
